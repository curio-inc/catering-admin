...
export default async function OrdersProductionPage({ searchParams }: { searchParams: SearchParams }) {
    ...
    <div className="min-h-screen">
      <header className="border-b border-amber-200 bg-gradient-to-r from-amber-500 to-orange-600 text-white">
        <div className="mx-auto flex max-w-6xl flex-col gap-1 px-4 py-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-medium opacity-90">{brand.displayName}</p>
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-6 text-amber-950">
            ...
            {isEnvIssue ? (
              <p className="mt-4 text-sm text-stone-700">
                Supabase を使わずデモ表示する場合は <code className="rounded bg-white px-1">USE_MOCK_DATA=true</code> を
                <code className="rounded bg-white px-1">.env.local</code> に追加してください。本番データを使う場合は
